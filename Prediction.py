...
def retrain(Param):
    ...
    Rdata=pickle.load(open('level'+str(level)+'/DATA_'+parent+'.p'))
    for d in data:
        Rdata.append(d)
#    pickle.dump(Rdata,open('level'+str(level)+'/DATA_'+parent+'.p','wb'))
    
    labelDict=pickle.load(open('level'+str(level)+'/amazon_'+parent+'_LABEL_DICT.p'))
    catMap=pickle.load(open('level'+str(level)+'/amazon_'+parent+'_CAT_MAP.p'))
    labelMap=pickle.load(open('level'+str(level)+'/amazon_'+parent+'_LABEL_MAP.p'))
...
#    pickle.dump(labelDict,open('level'+str(level)+'/amazon_'+parent+'_LABEL_DICT.p','wb'))
#    pickle.dump(labelMap,open('level'+str(level)+'/amazon_'+parent+'_LABEL_MAP.p','wb'))
#    pickle.dump(catMap,open('level'+str(level)+'/amazon_'+parent+'_CAT_MAP.p','wb'))
    data=[]
    labels=[]
    for d in Rdata:
        label=catMap[d[1]]
...
def readData(Titles, Crumbs, parent):
    ...
    # pickle.dump(labelMap,open('level'+str(level)+'/'+domain+'_'+parent+'_LABEL_MAP.p','wb'))
#
    examples=[]
    labels=[]
    limit=20
...
def hclassify(hdata, parent, level):
    ...
    for child in childData:
        ...
        # pickle.dump(retData,open('level'+str(level)+'/DATA_'+parent+'.p','wb'))
...
def loadObjects(parent,level):
    try:
        clf=pickle.load(open('level'+str(level)+'/'+sd+'_'+parent+'_CMAX.p'))
        vec=pickle.load(open('level'+str(level)+'/'+sd+'_'+parent+'_VMAX.p'))
        norm=pickle.load(open('level'+str(level)+'/'+sd+'_'+parent+'_NMAX.p'))
        return vec,norm,clf
...
def prediction(level1,level2,cat,parent,fout,rout):

#    print 'Level = ',level,'  ', 'Parent = ',parent,'\n\n'
    
    
    vec,norm,clf=loadObjects(parent,level2)
    if vec==-1:
        return '',-1
    try:    
        cmap=pickle.load(open('level'+str(level2)+'/'+sd+'_'+parent+'_CAT_MAP.p'))
    except (OSError,EnvironmentError):
        print 'here   Level = ',level,'  ', 'Parent = ',parent,'\n\n'
        return '',-1
...
#            print newCat,occ,'\n'
            ...
            if not flag:
                tmpScore=0.0
                if Map[level1].has_key(cat):
                    tmpScore=Map[level1][cat][2]
                if occ > tmpScore:
                    Map[level1][cat]=(level2,newCat,occ)
                    Scores[level1][cat]=(occ,d)
                    return newCat,occ

            elif level1==level2 or not Map[level1].has_key(cat) or (Scores[level1].has_key(cat) and (Scores[level1][cat][0]<0.35 or Scores[level1][cat][1]<0.6)):
...
#            print 'Reached ', cat
        ...
        newCat,score=prediction(level,level,cat,par[1],fout,rout)
        
        if score==-1:
            return
        score2=1
        i=1
        while score != -1 and level+i<maxLevels:
            newCat2,score2=prediction(level,level+i,cat,newCat,fout,rout)
            if score2<score:
                break
            else:
                score=score2
...
for level in range(maxLevels):
    ...
    fout=open('TaxonomyAlignment/reports/'+td+'_MatchReport'+str(level)+'.csv','wb')
    rout=open('TaxonomyAlignment/reports/'+td+'_Retraining'+str(level)+'.txt','wb')
    
    Cats= []
    for cat in Data[level]:
        Cats.append((cat,level))
    for cat in Cats:
        parallelPredict(cat,fout,rout)
    fout.close()
    rout.close()

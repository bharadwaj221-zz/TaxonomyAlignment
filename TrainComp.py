...
#        pout.close()
        ...
        for j in range(1):
            ...
            for i, (train, test) in enumerate(skf):
                 file='TaxonomyAlignment/reports/level'+str(level)+'/'+domain+'_report_'+parent+'_'+str(i+1)+'.txt'
#                 vout=open(file,'wb')
...
tcout=open(domain+'_TrainComp.txt','wb')
...
compInterval=time.time()-compStart
tcout.write('Time taken for serial = '+str(compInterval)+ ' s\n')
...
#    tout.write('Classifiers = '+str(n)+' Time Taken = '+str(end-start)+ ' s')
#    tout.close()
print 'Completed parallel 2'

compInterval=time.time()-compStart
tcout.write('Time taken for parallel 2 = '+str(compInterval)+ ' s\n')
...
#    tout.write('Classifiers = '+str(n)+' Time Taken = '+str(end-start)+ ' s')
#    tout.close()
print 'Completed parallel 4'


compInterval=time.time()-compStart
tcout.write('Time taken for parallel 4 = '+str(compInterval)+ ' s\n')
...
compInterval=time.time()-compStart
tcout.write('Time taken for parallel 8 = '+str(compInterval)+ ' s\n')
tcout.close()
print 'Completed...'
